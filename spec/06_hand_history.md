...
- 一覧から Replay や Hand Gallery 投稿へ遷移できるようにする
...
6. `participants.role`、`participants.netResultPoints` など表示に必要な参加者情報を保持する
7. `result.winners`, `result.handValues`, `result.pot`, `result.streetEnded` を表示用に引き継ぐ
8. 変換後データを `history` 配列として画面に保持する

## 画面生成の概要
- Hand History 画面は `HistoryView` で描画する
- `history` 配列を順に `HandCard` へ渡して一覧表示する
- 一覧カードは対象プレイヤー基準で以下を表示する
  - プレイヤー名
  - 結果サマリ `Win / Lose / Chop`
  - 増減ポイント
  - ポジション
  - Hand
  - BoardCards
- 履歴データが存在しない場合は空表示メッセージを出す

## 履歴カードの基準プレイヤー
- Hand History では閲覧中ユーザーを基準プレイヤーとして扱う
- 表示名、結果、増減ポイント、ポジションは基準プレイヤーの `participants` 情報から表示する
